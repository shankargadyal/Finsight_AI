...
let C = { cyan:'#2F6FED', green:'#16A34A', red:'#DC2626', amber:'#CA8A04', purple:'#7C3AED', white:'#0F172A' };
const C_LIGHT = { cyan:'#2F6FED', green:'#16A34A', red:'#DC2626', amber:'#CA8A04', purple:'#7C3AED', white:'#0F172A' };
const C_DARK  = { cyan:'#5B8DEF', green:'#22C55E', red:'#F87171', amber:'#F59E0B', purple:'#A78BFA', white:'#E7ECF7' };
...
async function sendChat() {
  ...
  try {
    ...
    hideTyping();
    appendChatMsg('assistant', d.reply, d.sources);
  } catch(e) {
    hideTyping();
...
function appendChatMsg(role, text, sources) {
  ...
  const html = text
    ...
    .replace(/\n/g, '<br/>');
  div.innerHTML = `<div class="chat-bubble">${html}${renderRagSources(sources)}</div>`;
...
═══════════════════════════════════════════════════════════════ */
function initTheme() {
  const saved = localStorage.getItem('fsTheme') || 'light';
  applyTheme(saved);
}
function applyTheme(mode) {
  document.documentElement.setAttribute('data-theme', mode);
  localStorage.setItem('fsTheme', mode);
  Object.assign(C, mode === 'dark' ? C_DARK : C_LIGHT);
  const btn = $('themeToggleBtn');
  if (btn) btn.textContent = mode === 'dark' ? '☀️' : '🌙';
  // Re-render whatever's currently on screen with the new palette
  if (currentData) {
    try { renderTickerHeader(currentData); renderMainChart(currentData, period); } catch {}
  }
}
...
function initSidebarCollapse() {
  const saved = localStorage.getItem('fsSidebarCollapsed') === '1';
  if (saved) $('sidebarNav')?.classList.add('collapsed');
}
function toggleSidebar() {
  const nav = $('sidebarNav');
  if (!nav) return;
  nav.classList.toggle('collapsed');
  localStorage.setItem('fsSidebarCollapsed', nav.classList.contains('collapsed') ? '1' : '0');
}

/* ══════════════════════════════════════════════════════════════
   EXPLAINABILITY PANEL  (lightweight linear-SHAP)
═══════════════════════════════════════════════════════════════ */
async function loadExplainability(symbol) {
  const card = $('explainCard');
  const body = $('explainBody');
  if (!card || !body) return;
  card.style.display = '';
  body.innerHTML = '<div style="color:var(--muted);font-size:.8rem">Loading explanation…</div>';
  try {
    const d = await apiFetch(`/api/explain/${symbol}`);
    const rows = (list, cls, sign) => list.map(f => `
      <div class="explain-row ${cls}">
        <span class="explain-sign">${sign}</span>
        <span><span class="explain-feat">${f.feature}</span> — <span class="explain-note">${f.note}</span></span>
      </div>`).join('');
    body.innerHTML = `
      <div style="font-size:.8rem;color:var(--dim);margin-bottom:.6rem">
        Predicted next-day return: <strong style="color:${d.predicted_next_day_return_pct>=0?'var(--green)':'var(--red)'}">
        ${d.predicted_next_day_return_pct>=0?'+':''}${d.predicted_next_day_return_pct}%</strong>
      </div>
      ${rows(d.top_positive_features, 'positive', '▲')}
      ${rows(d.top_negative_features, 'negative', '▼')}
      <div class="explain-disclaimer">${d.disclaimer}</div>`;
  } catch (e) {
    body.innerHTML = `<div style="color:var(--muted);font-size:.8rem">Explainability unavailable for this symbol yet (needs more price history).</div>`;
  }
}

/* ══════════════════════════════════════════════════════════════
   RAG SOURCE CHIPS  (chat)
═══════════════════════════════════════════════════════════════ */
function renderRagSources(sources) {
  if (!sources || !sources.length) return '';
  const chips = sources.map(s => `<span class="rag-source-chip">${s.source}</span>`).join('');
  return `<div class="rag-sources">${chips}</div>`;
}
